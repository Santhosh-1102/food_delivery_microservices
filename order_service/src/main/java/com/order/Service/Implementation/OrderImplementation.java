...
public class OrderImplementation implements OrderService{
	...
	@Override
	public OrderDTO getMenu(Integer restaurentId,OrderRequest request) {
	 ApiResponse< List<MenuItemResponse>> orderMenu = restaurentClient.getMenu(restaurentId);
	 
	 List<MenuItemResponse> menu = orderMenu.getData();
		
		Map<Integer, MenuItemResponse> menuMap=menu.stream()
															.collect(Collectors
